...
def apply_bh_fdr(p_values: np.ndarray) -> np.ndarray:
    ...
    NaN entries are ignored (returned as NaN) and excluded from the correction,
    so a single NaN p-value does not poison the whole vector.
    """
...
def process_focal_cell_rra(
    ...
    # ---------------------------------------------------------------------
    # SECTION 5: EFFECT SIZE AND GENE SCORE
    # ---------------------------------------------------------------------
