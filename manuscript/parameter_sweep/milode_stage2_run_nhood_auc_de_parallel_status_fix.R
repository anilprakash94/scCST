...
preprocess_dir <- file.path(base_dir, "milode_hvg_harmony_rds")
preprocess_manifest <- file.path(preprocess_dir, "milode_hvg_harmony_preprocess_manifest.csv")
results_dir <- file.path(base_dir, "milode_parameter_sweeps_results_from_hvg_rds")
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

nhood_sizes <- c(10, 20, 30)
order_value <- 2
...
outer_workers <- as.integer(Sys.getenv("MILODE_OUTER_WORKERS", "2"))
inner_cores <- as.integer(Sys.getenv("MILODE_INNER_CORES", "2"))
outer_backend <- Sys.getenv("MILODE_OUTER_BACKEND", "multicore") # multicore, snow, serial
inner_backend <- Sys.getenv("MILODE_INNER_BACKEND", "multicore") # multicore, snow, serial
