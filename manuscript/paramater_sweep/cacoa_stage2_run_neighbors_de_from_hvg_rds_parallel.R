...
preprocess_manifest <- file.path(preprocess_dir, "cacoa_hvg_harmony_preprocess_manifest.csv")

# Keep this default compatible with the existing benchmark code.
results_dir <- file.path(base_dir, "cacoa_multi_results_parameter_sweeps")
...
outer_workers <- as.integer(Sys.getenv("CACOA_OUTER_WORKERS", "12"))
outer_backend <- Sys.getenv("CACOA_OUTER_BACKEND", "multicore") # multicore or serial
n_cores <- as.integer(Sys.getenv("CACOA_DE_CORES", "2"))
