...
manifest_path <- file.path(export_dir, "cacoa_milode_export_manifest_parameter_sweeps.csv")
...
preprocess_workers <- as.integer(Sys.getenv("MILODE_PREPROCESS_WORKERS", "2"))
preprocess_backend <- Sys.getenv("MILODE_PREPROCESS_BACKEND", "multicore") # multicore, snow, serial
overwrite <- FALSE
test_mode <- FALSE
...
summary_file <- file.path(preprocess_dir, "milode_hvg_harmony_preprocess_manifest.csv")
